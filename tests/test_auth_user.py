...
def test_login_success(user_payload):
    ...
    resp = UserMethods.login({"email": user_payload["email"], "password": user_payload["password"]})
...
def test_login_incorrect_data():
    resp = UserMethods.login({"email": "nope@example.com", "password": "wrong"})
